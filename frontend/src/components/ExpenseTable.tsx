...
export default function ExpenseTable({
  ...
  const [editValues, setEditValues] = useState<{ category: string; amount: number }>({
    category: '',
    amount: 0,
  });
  ...
  const handleEdit = (expense: Expense) => {
    ...
    setEditValues({ category: expense.category, amount: expense.amount });
  };
  ...
  // ← FIXED: safely format date, avoid "Invalid Date" when date is null
  const formatDate = (dateStr: string | null | undefined): string => {
    if (!dateStr) return '—';
    try {
      // Append T00:00:00 to prevent timezone shift on Mac/Safari
  ...
  // ← FIXED: show a friendly fallback when merchant is still "Processing..."
  const formatMerchant = (merchant: string | null | undefined, expenseStatus: string): string => {
    if (!merchant || merchant === 'Processing...') {
      if (expenseStatus === 'processing' || expenseStatus === 'pending') {
        return 'Processing...';
      }
      return 'Unknown';
    }
          ...
          <tbody className="divide-y divide-slate-800/50">
            {sortedExpenses.length === 0 ? (
              ...
              sortedExpenses.map((expense) => (
                ...
                >
                  {/* ← FIXED: use formatDate() instead of new Date(...).toLocaleDateString() */}
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                    {formatDate(expense.date)}
                  </td>
                  {/* ← FIXED: use formatMerchant() to handle null/Processing... */}
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white font-medium">
                    {formatMerchant(expense.merchant, expense.status)}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    {editingId === expense.id ? (
                      <input
                        ...
                        className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-white w-24 focus:outline-none focus:border-blue-500"
                      />
                    ) : (
